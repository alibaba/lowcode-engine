...
| packageId | 对应的资产包对象      | String | -      | -      | 选填，页面对应的资产包对象，一般用于微前端场景下，低代码引擎中的微前端只规定了对应的场景，并不依赖于具体实现，详见下面 **packageId** 说明 |
| meta    | 页面元信息            | Object | -      | -      | 选填，用于描述当前应用的配置信息                      |
| config  | 页面配置              | Object | -      | -      | 选填，用于描述当前应用的元数据信息                     |


关于 **packageId** 的详细说明：
在微前端的场景下，可能会存在多个页面路由到同一个应用，应用可通过资产包加载，所以需要将对应的页面配置指向对应的微应用（资产包）对象。
