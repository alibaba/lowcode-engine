...
const pluginFactory: BuilderComponentPluginFactory<PluginConfig> = (config?) => {
  ...
  const plugin: BuilderComponentPlugin = async (pre: ICodeStruct) => {
    ...
    const mapComponentNameToAliasOrKeepIt =
      (componentName: string) => componentsNameAliasMap.get(componentName) || componentName;
    ...
    // 1. 小程序出码的时候，循环变量没法拿到
    // 2. 小程序出码的时候，很容易出现 Uncaught TypeError: Cannot read property 'avatar' of undefined
    //    这样的异常(如下图的 50 行) -- 因为若直接出码，Rax 构建到小程序的时候会立即计算所有在视图中用到的变量
    ...
    next.chunks.push({
      ...
      // TODO: setState, dataSourceMap, reloadDataSource, utils,
      //       i18n, i18nFormat, getLocale, setLocale
      //       这些在 Rax 的编译模式下不能在视图中直接访问，需要转化成 this.xxx
      content: `
...
function generateEventHandlerAttrForRax(
  ...
  const referencedLocalVariables =
    _.intersection(undeclaredVariablesInValueExpr, currentScopeVariables);
