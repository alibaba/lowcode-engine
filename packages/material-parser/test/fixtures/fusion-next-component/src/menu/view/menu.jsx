...
export default class Menu extends Component {
  ...
  adjustChildrenWidth() {
    ...
    const menuItemNodes = [].slice
      .call(children)
      .filter(
        node => node.className.split(' ').indexOf(`${prefix}menu-more`) < 0,
      );

    const overflowedItems = menuItemNodes.filter(
      c => c.className.split(' ').indexOf(MENUITEM_OVERFLOWED_CLASSNAME) >=
                0,
    );
      ...
      if (
        ...
        this.props.onOpen([], {
          key: this.state.openKeys.sort(
            (prevKey, nextKey) => this.k2n[nextKey].pos.split('-').length -
