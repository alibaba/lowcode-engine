...
export default class Tree extends Component {
  ...
  handleExpand(expand, key, node) {
    ...
    const { expandedKeys } = this.state; // 由于setState 是异步操作，所以去掉 [...this.state.expandedKeys]
        ...
        if (
          ...
          if (pnode.checkable === false) {
            ...
            for (let m = 0; m < pnode.children.length; m++) {
              if (
                !pnode.children.every(child => isNodeChecked(child, checkedKeys))
              ) {
                parentCheck = false;
                break;
              }
  ...
  getDropPosition(e, node) {
    ...
    const { offsetHeight } = labelWrapperNode;
    const { pageY } = e;
    const gapHeight = 2;
