...
export type OutputFuntion = (...data: any[]) => void;

const outputFunctionRecord: Record<string, OutputFuntion> = {
  debug: console.log,
  log: console.log,
  info: console.log,
  warn: console.warn,
...
const output = (logLevel: string, bizName: string) => {
  return (...args: any[]) => {
    return outputFunctionRecord[logLevel]?.apply(console, getLogArgs(args, bizName, logLevel));
  };
};
