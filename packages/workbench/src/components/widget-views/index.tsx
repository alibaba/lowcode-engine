...
export class DraggableLineView extends Component<{ panel: Panel }> {
  ...
  onDrag(value: number) {
    ...
    // 抛出事件，对于有些需要 panel 插件随着变化进行再次渲染的，由panel插件内部监听事件实现
  ...
  render() {
    ...
    if (
      isRightArea ||
      !enableDrag ||
      this.props.panel?.parent?.name === 'leftFixedArea'
    ) {
      return null;
    }
    return (
...
@observer
export class TitledPanelView extends Component<{
  panel: Panel;
  area?: string;
}> {
    ...
    return (
      <div
        ...
        data-keep-visible-while-dragging={
          panel.config.props?.keepVisibleWhileDragging
        }
      >
  ...
  render() {
    ...
    if (this.props.shouldHideSingleTab && container.items.length === 1) {
      ...
      container.items.forEach((item: any) => {
        titles.push(
          <PanelTitle key={item.id} panel={item} className="lc-tab-title" />,
        );
        contents.push(
          <PanelView
            key={item.id}
